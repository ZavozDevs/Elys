...
## 📜 Previous Base (Heroku 2.1.0)
 ...
 - add -w and --wipe argument for python3 -m elys for wipe session, databases, loaded modules
...
- fix close btn
- fix native imports
- added converting Hikka backups to Elys
...
- added config for show "Elys" in info
- removed UnitHeta
...
- https://github.com/ZavozDevs/Elys/issues/115
- https://github.com/ZavozDevs/Elys/issues/114
- https://github.com/ZavozDevs/Elys/issues/119
- https://github.com/ZavozDevs/Elys/issues/120
- Other changes
...
- Drop Okteto support
- Show Elys platform and version in Telegram sessions list
...
- Completely drop Elys support due to legacy code, limits and removing of free tier
...
- Publish hikka telethon fork and migrate to it in requirements, thereby fixing the deployment error on Elys
- Add custom emojis filter to `utils.remove_html`
- Fix `client.get_perms_cached`
- Fix translation flaw in `ElysSecurity`
...
- Add platform-specific Elys emojis to premium users
...
- Add support for multiaccounting on Elys
...
- Update quickstart
- New fields in ElysInfo
- Remove suffix `Elys` if user specified it somewhere below
...
- Add `banner_url` config var to `ElysInfo`
- Add `loader.validators.Hidden`
- Add `websockets` dependency, so users can load hikarichat on Elys
...
- Add `client.force_get_entity` to bypass Elys Cacher
...
- Fix translation issue in ElysConfig
- Fix `.dump`
...
- New Elys dependencies list in Pipfile
...
- Add banner with Elys installation status
- Reorder config saving in web
- Fix `Unauthorized` error on Elys
...
- Add full-featured Elys support with additional buildpacks
...
- Drop Uniborg and Raphielgang compatibility layer. More info in [2d525ab](https://github.com/hikariatama/Elys/commit/2d525ab6b0e6b9d7ccd7408bbe175cea24d780a5)
- Minor bug fixes

## 🌑 Elys 1.1.0

- Make inline commands' docstrings translatable
- Introducing ElysDynamicTranslate - the possibility for developers to translate their own modules to other language without usage of native translates
...
- Refactor ElysSecurity, especially the bounding mask control
...
- Add ability to disable ElysDL natively
- Fix Updater so it works if you have troubles with inline mode

## 🌑 Elys 1.0.22

Add ElysDL module to use inline download buttons in verified channels
