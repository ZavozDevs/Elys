...
class ElysBackupMod(loader.Module):
        ...
        try:
            ...
            with zipfile.ZipFile(zipfile_bytes) as zf:
                with zf.open("db.json") as f:
                    raw_db = f.read().decode()
                    if re.search(r'"(?:heroku|hikka|legacy|ftg)\.', raw_db):
                        raw_db = re.sub(r'"(?:heroku|hikka|legacy|ftg)\.(\S+":)', r'"elys.\1', raw_db)
                    db_data = self._db.migrate_data(orjson.loads(raw_db))
    ...
    async def restoredb(self, message: Message):
        ...
        if re.search(r'"(?:heroku|hikka|legacy|ftg)\.', raw_content):
            raw_content = re.sub(r'"(?:heroku|hikka|legacy|ftg)\.(\S+":)', r'"elys.\1', raw_content)

        decoded_text = self._db.migrate_data(orjson.loads(raw_content))
        ...
        try:
            ...
            with zipfile.ZipFile(zipfile_bytes) as zf:
                ...
                with contextlib.suppress(KeyError):
                    db_data["elys.inline"].pop("bot_token")
