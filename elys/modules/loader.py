...
class LoaderMod(loader.Module):
    ...
    async def load_module(
        ...
        if elys_min:
            ...
            if main.__version__ < ver_:
                incompatible_ver = ver

        elif legacy_min:
            ver = legacy_min.group(1)
            ver_ = tuple(map(int, ver.split(".")))
            # Elys 1.0.0 is based on Heroku 2.1.0 and fully supports heroku/hikka modules <= 2.1.0
            if ver_ > (2, 1, 0):
                incompatible_ver = ver

        if incompatible_ver:
            logger.error(
                "Module %s requires version %s, current version is %s",
                module_label,
                incompatible_ver,
        ...
        module_name = f"elys.modules.{uid}"
        ...
        try:
            developer_entity = await (
                ...
                if (
                    developer in self._client.elys_entity_cache
                    and getattr(
    ...
    def _is_core_module(self, module) -> bool:
        ...
        if not module_name.startswith("elys.modules."):
            return False
