...
class UpdaterMod(loader.Module):
    ...
    async def poller_announcement(self):
        async with aiohttp.ClientSession() as session:
            try:
                url = "https://raw.githubusercontent.com/ZavozDevs/assets/main/elys_userbot/announcment.txt"
                r = await session.get(
                    url,
    ...
    async def update_call(self, call: InlineCall):
        ...
        if NO_GIT:
            ...
            return
        if call.data not in {"elys/update", "elys/ignore_upd"}:
            return

        if call.data == "elys/ignore_upd":
            ...
            try:
                await self._client(
                    UpdateDialogFilterRequest(
                        ...
                        DialogFilter(
                            folder_id,
                            title=TextWithEntities(text="Elys", entities=[]),
                            pinned_peers=(
                                [
                ...
                logger.critical(
                    "Can't create Elys folder. Possible reasons are:\n"
