...
class Evaluator(loader.Module):
    ...
    class _SecureDB:
        ...
        def set(self, *args, **kwargs):
            if len(args) >= 2 and args[0] == "elys.security" and args[1] == "owner":
                raise ValueError(
    ...
    def censor(self, ret: str) -> str:
        ret = ret.replace(str(self._client.elys_me.phone), "&lt;phone&gt;")
        ...
        if btoken := self._db.get("elys.inline", "bot_token", False):
            ret = ret.replace(
                btoken,
                f'{btoken.split(":")[0]}:{"*" * 26}',
            )
