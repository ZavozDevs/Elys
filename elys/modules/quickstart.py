...
class Quickstart(loader.Module):
    ...
    async def client_ready(self):
        self.text = lambda: self.strings["base"].format(
            ...
            if self.client.elys_me.premium is True
            else "Elys"
        )

        try:
            content_channel = None
            existing_channel_id = self.db.get("elys.forums", "channel_id", None)
            ...
            forum_entity = None
            existing_forum_id = self.db.get("elys.forums", "forum_id", None)
            ...
            required_topics = [
                ...
                (
                    "Backups",
                    "💾 Your Elys backups will be stored here",
                    5877307202888273539,
                ),
            ]
    ...
    async def lang(self, call: BotInlineCall):
        if not call.data.startswith("elys/lang/"):
            return
