...
class TestMod(loader.Module):
        ...
        self.config = loader.ModuleConfig(
            loader.ConfigValue(
                ...
                (
                    ...
                    " Elys will try to determine, which client caused logs. E.g. there"
                    ...
                    " Elys will send all logs to Client1 and Client2, even if it is"
    ...
    async def logs(
        ...
        other = (
            *main.__version__,
            (
                " <a"
                f' href="https://github.com/ZavozDevs/Elys/commit/{ghash}">@{ghash[:8]}</a>'
                if ghash
                else ""
            ),
        )
    ...
    @loader.command(
        ru_doc="<html текст> - Отправить или отредактировать сообщение как Rich Message",
        ua_doc="<html текст> - Надіслати або відредагувати повідомлення як Rich Message",
        en_doc="<html text> - Send or edit message as Rich Message",
    )
    async def rich(self, message: Message):
        """<html text> - Send or edit message as Rich Message"""
        ...
        if not args:
            await utils.answer(
                message,
                "<tg-emoji emoji-id=5210952531676504517>❌</tg-emoji> <b>Укажи HTML текст или ответь на сообщение</b>",
            )
            return
    ...
    async def getrich(self, message: Message):
        ...
        if not rich_html and getattr(target, "rich_message", None):
            try:
                ...
                rich_html = rich_message_to_html(target.rich_message)
            except Exception:
                pass
        ...
        if not rich_html:
            await utils.answer(
                message,
                "<tg-emoji emoji-id=5210952531676504517>❌</tg-emoji> <b>Сообщение не является Rich Message или не удалось извлечь HTML.</b>",
            )
            return
        ...
        if len(rich_html) > 3000:
            ...
            await utils.answer(
                message,
                "<b>Rich Message HTML:</b>",
                file=file,
            )
        else:
            await utils.answer(
                message,
                f"<b>Rich Message HTML:</b>\n\n<code>{utils.escape_html(rich_html)}</code>",
            )
