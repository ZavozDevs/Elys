    ...
    async def create_topic() -> ForumTopic:
        result = await client(
            CreateForumTopicRequest(
                peer=entity,
                title=title,
                icon_emoji_id=(icon_emoji_id if client.elys_me.premium else None),
            )
        )
...
async def wait_for_content_channel(db: "Database", delay: float = 10) -> int:
    cid = db.get("elys.forums", "channel_id", None)

    while not cid:
        logger.warning(
            "Elys content channel not found in database. Sleeping 10 seconds..."
        )
