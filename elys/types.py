...
class Module:
    ...
    @property
    def elys_commands(self) -> dict[str, Command]:
        """List of commands that module supports"""
        return get_commands(self)
    ...
    @property
    def elys_inline_handlers(self) -> dict[str, Command]:
        """List of inline handlers that module supports"""
        return get_inline_handlers(self)
    ...
    @property
    def elys_callback_handlers(self) -> dict[str, Command]:
        """List of callback handlers that module supports"""
        return get_callback_handlers(self)
    ...
    @property
    def elys_watchers(self) -> dict[str, Command]:
        """List of watchers that module supports"""
        return get_watchers(self)
    ...
    @elys_commands.setter
    def elys_commands(self, _):
        pass
    ...
    @elys_inline_handlers.setter
    def elys_inline_handlers(self, _):
        pass
    ...
    @elys_callback_handlers.setter
    def elys_callback_handlers(self, _):
        pass
    ...
    @elys_watchers.setter
    def elys_watchers(self, _):
        pass
    ...
    async def _decline(
        ...
        self._db.set(
            "elys.main",
            "declined_joins",
            list(set(self._db.get("elys.main", "declined_joins", []) + [channel.id])),
        )
        event.status = False
        event.set()
    ...
    async def request_join(
        ...
        try:
            ...
            if assure_joined:
                raise LoadError(f"Channel @{peer} not found") from e
            return False
        ...
        match channel:
            ...
            case _ if channel.id in self._db.get("elys.main", "declined_joins", []):
                if assure_joined:
                    raise LoadError(
        ...
        self.elys_wait_channel_approve = (
            self.__class__.__name__,
            channel,
            reason,
        )
        ...
        with contextlib.suppress(AttributeError):
            delattr(self, "elys_wait_channel_approve")
        ...
        *,
        ...
        if elys_lib_min:
            ...
            if version.__version__ < ver:
                _raise(
                    RuntimeError(
                        f"Library requires Elys version {'{}.{}.{}'.format(*ver)}+"
                    )
                )
        elif legacy_lib_min:
            ver = tuple(map(int, legacy_lib_min[1].split(".")))
            if ver > (2, 1, 0):
                _raise(
                    RuntimeError(
        ...
        if (
            ...
            and self._db.get("elys.main", "stats", True)
            and url is not None
