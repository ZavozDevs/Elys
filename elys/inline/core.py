...
class InlineManager(
    ...
    def __init__(
        ...
        self._token = (
            db.get("elys.inline", "bot_token", False)
            or db.get("heroku.inline", "bot_token", False)
            or db.get("hikka.inline", "bot_token", False)
            or False
        )
    ...
    async def register_manager(
        ...
        self._name = get_display_name(self._client.elys_me)
        ...
        self._bot_client = TelegramClient(
            SQLiteSession(
                os.path.join(main.SESSIONS_DIR, f"elys-{self._me}-bot-{bot_uid}")
            ),
            self._client.api_id,
        ...
        try:
            ...
            self._bot_client.elys_me = bot_me
    ...
    async def _ping_bot(
        ...
        try:
            ...
            self._db.set("elys.inline", "bot_token", None)
            self._token = False
            ...
            try:
                m = await self._client.send_message(
                    self.bot_username, "/start elys init"
                )
            except Exception:
