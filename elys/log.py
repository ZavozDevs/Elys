...
def getlines(filename: str, module_globals=None) -> str:
    ...
    source code of Elys modules properly.
    """
...
class TelegramLogsHandler(logging.Handler):
    ...
    def dumps(
        ...
        return [
            ...
            and (not record.elys_caller or client_id == record.elys_caller)
        ]
    ...
    async def sender(self):
        async with self._send_lock:
            ...
            for client_id in self._mods:
                ...
                funcs = []
                for item in self.tg_buff:
                    if not isinstance(item[0], ElysException):
                        continue
    ...
    def emit(self, record: logging.LogRecord):
        try:
            caller = next(
                (
                    frame_info.frame.f_locals["_elys_client_id_logging_tag"]
