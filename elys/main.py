...
class Elys:
    ...
    def _migrate_sessions(self):
        ...
        with os.scandir(BASE_DIR) as entries:
            legacy = [
                ...
                if entry.is_file()
                and (entry.name.startswith("elys-") or entry.name.startswith("heroku-"))
    ...
    async def save_client_session(
        ...
        *,
        ...
        client.elys_db = database.Database(client)
        await client.elys_db.init()
    ...
    async def _badge(self, client: CustomTelegramClient):
        ...
        try:
            ...
            pref = client.elys_db.get("elys.main", "command_prefix", None)
            ...
            try:
                ...
                await client.elys_inline.bot.send_photo(
                    log_chat_id,
                    "https://raw.githubusercontent.com/ZavozDevs/assets/main/elys/elys_started.png?v=1",
                    caption=(
